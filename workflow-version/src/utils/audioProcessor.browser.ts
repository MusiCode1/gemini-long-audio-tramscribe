import { CHUNK_DURATION_MIN, OVERLAP_DURATION_MIN } from '../constants.js';
...
/**
 * מייצג את מקור האודיו, בין אם מדפדפן או ממערכת הקבצים.
 */
...
/**
 * מפענח ArrayBuffer של אודיו ל-AudioBuffer, עם תאימות לסביבת Node.js ודפדפן.
 */
function decodeAudioData(arrayBuffer: ArrayBuffer): Promise<AudioBuffer> {
    debugLog('Starting audio decode...', { byteLength: arrayBuffer.byteLength });
    return new Promise((resolve, reject) => {
        // ה-polyfill של web-audio-api משתמש ב-callbacks במקום להחזיר Promise.
        // לכן, אנו עוטפים את הקריאה ב-Promise כדי ליצור API אחיד.
        audioContext?.decodeAudioData(
            arrayBuffer,
            (audioBuffer) => { // successCallback
                if (!audioBuffer) {
                    // במקרה שה-callback נקרא ללא שגיאה אך גם ללא buffer
                    reject(new Error("Failed to decode audio data, buffer is null."));
                    return;
                }
            ...
            (error) => { // errorCallback
                console.error("Error decoding audio data with web-audio-api:", error);
                reject(error || new Error("An unknown error occurred during audio decoding."));
            }
        );
    });
}
 ...
 * @param audioSource אובייקט המכיל את נתוני האודיו.
 * @param onProgress פונקציית callback לדיווח על התקדמות.
 * @returns אובייקט עם המספר הכולל של המקטעים שנוצרו.
 */
export async function chunkAndStoreAudio(audioSource: AudioSource, onProgress: (message: string) => void): Promise<{ totalChunks: number }> {
    onProgress('מפענח את קובץ האודיו...');
    let originalBuffer: AudioBuffer | null = await decodeAudioData(audioSource.arrayBuffer);
    ...
    if (duration <= chunkDurationSec) {
        ...
        // שימוש ב-Blob במקום ב-File כדי לשמור על תאימות בין סביבות
        await saveChunk(0, wavBlob, `chunk_0.wav`);
    ...
    while (currentTime < duration) {
        ...
        let chunkBuffer: AudioBuffer | undefined = audioContext?.createBuffer(numberOfChannels, frameCount, sampleRate);
        ...
        for (let i = 0; i < numberOfChannels; i++) {
            ...
            // תיקון: שימוש ב-set במקום ב-copyToChannel שאינו נתמך בפוליפיל
            (chunkBuffer as AudioBuffer).getChannelData(i).set(slicedData);
        }

        // קידוד המקטע ל-WAV ושמירתו ב-IndexedDB
        const wavBlob = encodeWAV(chunkBuffer!);
        chunkBuffer = undefined; // שחרור ה-buffer של המקטע מהזיכרון
